...
final class ZipCryptoCipher {
  ...
  ZipCryptoCipher(String password) {
...
WinZipAesPayload encryptWinZipAes({
  ...
  final Uint8List derived = pbkdf2Sha1(utf8.encode(password), salt, iterations: 1000, length: keyLength * 2 + 2);
  ...
  final Uint8List authentication = Uint8List.sublistView(hmacSha1(authenticationKey, encrypted), 0, 10);
  final Uint8List result = Uint8List(salt.length + 2 + encrypted.length + 10)
    ..setRange(0, salt.length, salt)
    ..setRange(salt.length, salt.length + 2, verifier)
    ..setRange(salt.length + 2, salt.length + 2 + encrypted.length, encrypted)
    ..setRange(salt.length + 2 + encrypted.length, salt.length + 2 + encrypted.length + 10, authentication);
  return WinZipAesPayload(result);
}

/// Decrypts and authenticates one WinZip AES AE-1 or AE-2 payload.
Uint8List decryptWinZipAes({required List<int> payload, required String password, required int keyLength}) {
  final int saltLength = keyLength ~/ 2;
  final int overhead = saltLength + 2 + 10;
  if (payload.length < overhead) {
    throw const ZCodecException('Truncated WinZip AES payload');
  }
  final Uint8List salt = Uint8List.fromList(payload.sublist(0, saltLength));
  final Uint8List derived = pbkdf2Sha1(utf8.encode(password), salt, iterations: 1000, length: keyLength * 2 + 2);
  final Uint8List verifier = Uint8List.sublistView(derived, keyLength * 2);
  if (!_constantTimeEquals(verifier, payload.sublist(saltLength, saltLength + 2))) {
    throw const ZCodecException('Incorrect ZIP password');
  }
  final Uint8List authenticationKey = Uint8List.sublistView(derived, keyLength, keyLength * 2);
  final int encryptedStart = saltLength + 2;
  final int encryptedEnd = payload.length - 10;
  final Uint8List encrypted = Uint8List.fromList(payload.sublist(encryptedStart, encryptedEnd));
  final Uint8List expectedAuthentication = Uint8List.sublistView(hmacSha1(authenticationKey, encrypted), 0, 10);
  if (!_constantTimeEquals(expectedAuthentication, payload.sublist(encryptedEnd))) {
    throw const ZCodecException('Invalid WinZip AES authentication code');
  }
  final Uint8List encryptionKey = Uint8List.sublistView(derived, 0, keyLength);
  return AesCipher(encryptionKey).cryptWinZipCtr(encrypted);
}
